...
interface QuestionRepo : JpaRepository<QuestionEntity, Long> {
    ...
    @Query(value = "SELECT * FROM your_table_name ORDER BY RANDOM() LIMIT ?1", nativeQuery = true)
    fun findRandomRecordsPostgres(count: Int): List<QuestionEntity>

    // Define a method for H2
    @Query(value = "SELECT * FROM your_table_name ORDER BY RAND() LIMIT ?1", nativeQuery = true)
